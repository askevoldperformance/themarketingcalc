...
document.querySelectorAll('.channel-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    if (selectedChannels.has(ch) && selectedChannels.size > 1) {
      selectedChannels.delete(ch); btn.classList.remove('active');
    } else if (!selectedChannels.has(ch)) {
      selectedChannels.add(ch); btn.classList.add('active');
    }
    renderBenchmarks();
  });
});
  ...
  channels.forEach(ch => {
    ...
    if (selectedDirection === 'budget-to-results') {
      ...
      if (selectedObjective === 'reach') {
        html += `<div style="margin-bottom:10px"><strong>${CHANNEL_NAMES[ch]}:</strong> ~${fmtInt(perChannel / b.cpm * 1000)} impressions (CPM: ${sym}${fmtNum(b.cpm)})</div>`;
      } else if (selectedObjective === 'clicks') {
        html += `<div style="margin-bottom:10px"><strong>${CHANNEL_NAMES[ch]}:</strong> ~${fmtInt(perChannel / b.cpc)} clicks (CPC: ${sym}${fmtNum(b.cpc)})</div>`;
      } else {
        html += `<div style="margin-bottom:10px"><strong>${CHANNEL_NAMES[ch]}:</strong> ~${fmtInt((perChannel / b.cpc) * 0.02)} conversions at 2% CVR (CPC: ${sym}${fmtNum(b.cpc)})</div>`;
      }
    } else {
      const goal = getVal('g2b-goal');
      if (!goal) return;
      let needed = 0;
      if (selectedObjective === 'reach') needed = goal / 1000 * b.cpm;
      else if (selectedObjective === 'clicks') needed = goal * b.cpc;
      else needed = goal / 0.02 * b.cpc;
      html += `<div style="margin-bottom:10px"><strong>${CHANNEL_NAMES[ch]}:</strong> ~${sym}${fmtNum(needed)} required</div>`;
    }
  });
  result.innerHTML = `<div class="result-label">Estimated results</div>${html}<div style="font-size:0.78rem;color:#64748B;margin-top:8px">Estimates based on market benchmarks. Actual results vary by creative, audience, and bidding.</div>`;
}
...
function renderBenchmarks() {
  ...
  Object.keys(CHANNEL_NAMES).forEach(ch => {
    const b = data[ch];
